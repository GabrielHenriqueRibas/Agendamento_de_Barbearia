...
class ProblemsController extends Controller
{
    public function index(Request $request): void
    {
        ...
        $title1 = 'Area do Usuario';
        $title2 = 'Agendamentos';

        if ($request->acceptJson()) {
            $this->renderJson('problems/index', compact('paginator', 'problems', ));
        } else {
            $this->render('problems/index', compact('paginator', 'problems', 'title1', 'title2', 'route'));
        }
    }

    public function admin(Request $request): void
    {
        $this->render('admin/index', compact('admin', 'title'));
    }
